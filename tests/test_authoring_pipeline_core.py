...
def test_transport_api_blocks_on_project_gate(tmp_path: Path, monkeypatch):
    view = SimpleNamespace(
        id="demo",
        title="Demo",
        canonical_ref="agent/demo",
    ...
    monkeypatch.setattr(authoring_pipeline, "_remove_worktree", lambda root, target: None)
    monkeypatch.setattr(
        authoring_pipeline.project_gates,
        "coverage",
        lambda project: {
            "schema_version": "spec_workbench_project_gates.v1",
            "ready": False,
            "summary": {
                "gates": 1,
                "ready_gates": 0,
            ...
            "results": [],
            "findings": [{"severity": "error", "code": "demo", "message": "blocked"}],
        },
    )
    monkeypatch.setattr(
        authoring_pipeline.design_authoring_next,
        "next_step",
        lambda case_root, *, display_path=None: {
            "schema_version": design_authoring_next.SCHEMA,
            "phase": "state6_exact_contracts",
            "blocked": False,
            "reason": "would continue",
            "action": {"tool": "tools/design_stage6_contracts.py", "args": [display_path], "command": "ignored"},
            "summary": {},
            "findings": [],
        },
    )
    ...
    result = authoring_pipeline.project_next(tmp_path, "demo")
    assert result["authoring"]["phase"] == "state6_exact_contracts"
    assert result["authoring"]["blocked"] is True
    assert result["authoring"]["action"]["tool"] == "tools/design_project_gates.py"
    assert result["authoring"]["action"]["args"] == ["examples/demo"]
    assert result["project_gates"]["ready"] is False
