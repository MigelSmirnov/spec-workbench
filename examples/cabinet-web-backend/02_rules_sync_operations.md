...
2. One delivery uses contract version `cabinet-web-sync-v1` and contains one
   exact M19 catalogue ID/hash, declared snapshot count, source/target nodes,
   idempotency identity, creation time, and its complete M18 snapshots in
   canonical `project_id` order. The hash is SHA-256 over canonical UTF-8 JSON
   with lexicographically sorted object keys. Cabinet Web validates identity,
   count, ordering, contract version, and content hash before acceptance.
