...
- contract version `cabinet-web-sync-v1`;
- catalogue ID, catalogue content hash, snapshot count, and ordered M18 project
  snapshots in canonical `project_id` order;
- source and target node IDs;
- caller-scoped idempotency identity;
- creation time.

The content hash is SHA-256 over canonical UTF-8 JSON with lexicographically
sorted object keys and the project snapshots already ordered by `project_id`.
