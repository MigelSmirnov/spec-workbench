...
Authenticated node and exact M24 delivery using `cabinet-web-sync-v1`, catalogue
ID/SHA-256 hash/count, canonical `project_id` ordering, endpoints, idempotency,
creation time, and ordered snapshots.
...
Partial, older or conflicting delivery never becomes current. Hash verification
uses canonical UTF-8 JSON with lexicographically sorted object keys.
