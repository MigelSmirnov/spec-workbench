...
refund, attachment, status interpretation, or later-revision reconciliation
behavior. Ambiguous create recovery is defined separately by A52.
...
Holded create is not assumed to be idempotent. Ambiguous create outcomes follow
the marker-based recovery protocol defined by A52.

Therefore:

1. Backend must not automatically repeat POST after an ambiguous timeout.
2. Every attempt has a stable local `publication_attempt_id` and exact marker.
3. Recovery uses bounded list polling and GET verification without mutation.
4. Zero marker matches do not prove that POST failed and do not authorize an
   automatic retry.
...
## Accepted decision A52 — marker-based Holded purchase create recovery

This decision is based on completed runtime discovery against the Holded
invoicing v1 purchase API. It defines recovery of an ambiguous purchase-create
outcome without repeating POST.
...
## Accepted decision A61 — separate machine and local-user identities
