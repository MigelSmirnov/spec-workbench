...
attach_local_source: [FIELD_ASSIGNMENT] MUST assign the source identity of a local file as file.expected_source_id when supplied, otherwise f"local:{sha256 hex of file.content}"; a SourceBinary already stored for the same invoice_id and content_hash is the same source: the item result is attached with that stored source_id, no second identity row is inserted, and a replica row is added only when this node has none for it.
