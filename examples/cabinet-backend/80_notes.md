...
get_transfer_receipt: [BEHAVIOR] Open one transaction, read list_transfer_receipts for the invoice, commit, and return the receipt whose manifest_hash equals the argument or None; no receipt is ever synthesized.
get_transfer_receipt: [DEPENDENCY_BOUNDARY] The only way another module reads a transfer receipt; callers never touch ArchiveUnitOfWork.
