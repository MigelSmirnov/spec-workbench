...
A Registry refresh holds the catalogue lock, derives the complete typed WorkObject replacement according to accepted Registry rules, replaces it in one PostgreSQL transaction, and returns a result describing that committed observation. Failure rolls back and preserves the previous committed catalogue.
