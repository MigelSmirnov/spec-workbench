...
- Каждая запись секции `models` принадлежит модулю `models`: её имя стоит в
  `module_functions["models"]`. Это предусловие детерминированной сборки
  модуля — при записи, отданной другому модулю, `models` целиком теряет
  детерминированную эмиссию и уходит на LLM-путь
...
- владелец интерфейса — модуль `models`: `module_functions["models"]` и
  `imports.internal["models"]` содержат его имя, как у любой другой записи
  секции `models`. Порт принадлежит ядру, а не адаптеру: модуль-реализация
  импортирует свой порт из `models` (`imports.module_internal[реализация]
  ["models"]`) и наследует его, а потребитель зависит от порта, не завися от
  адаптера. Назначение порта модулю-реализации — дефект спеки
...
19a. **Persistence IR нормативен?** Если присутствует `persistence_backend`,
     это версия 2; v1 отсутствует. Aggregate покрыт root/one/many без списка
     шагов, а `codec_coverage_gap` перечисляет разрывы покрытия.
